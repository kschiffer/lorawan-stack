...
describe('gateways reducer', function () {
  ...
  it('should ignore `getGatewaysFailure` action', function () {
    expect(reducer(defaultState, getGatewaysFailure({ status: 404 }))).toEqual(defaultState)
  })
    ...
    describe('receives gateway', function () {
      ...
      describe('requests a list of gateways', function () {
        ...
        describe('receives a list of gateways', function () {
          ...
          beforeAll(function () {
            newState = reducer(newState, getGatewaysSuccess({ entities, totalCount }))
          })

          it('should not remove previously received gateway on `getGatewaysSuccess` action', function () {
            expect(newState.entities[testGatewayId]).toEqual(testGateway)
          })

          it('should add new gateways to `entities` on `getGatewaysSuccess`', function () {
